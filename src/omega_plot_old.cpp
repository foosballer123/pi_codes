...
int main(int argc, char** argv)
{
    ...
    std::vector<double> omega_h;
    std::vector<double> omega_d_h;
    ...
    while (ros::ok() && !stop_node)
    {
        ...
        if (encoder_state == HIGH && prev_state == LOW)
        {
            ...
            if (delta_t > 0.0)
            {
                ...
                omega_h.push_back(omega);
                omega_d_h.push_back(omega_d);
            }
    ...
    gp << "plot '-' with lines\n";
    gp.send1d(boost::make_tuple(omega_h, omega_d_h));
