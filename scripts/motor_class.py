...
class Motor:
    ...
    def step(self, omega_d, steps = 1):
        ...
        if steps > 1:
            for _ in range(steps):
                ...
                dir, s = self.bounds(omega_d) # negative direction is away from camera 
                pt = self.get_pt(omega_d)
    ...
    def encoder(self):
        ...
        if (self.num == 1) or (self.num == 3):
            ...
            right = GPIO.input(md.motor[self.num]["SENSOR R"])
            left = GPIO.input(md.motor[self.num]["SENSOR L"])
            return right, left
