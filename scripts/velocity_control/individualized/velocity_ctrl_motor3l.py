...
if __name__ == '__main__':
    ...
    pos_pub = rospy.Publisher("/motor3_pos", Float32, queue_size=10)
    vel_pub = rospy.Publisher("/motor3_vel", Float32, queue_size=10)
    right_enc_pub = rospy.Publisher("motor3_right_encoder", Float32, queue_size=10)
    left_enc_pub = rospy.Publisher("motor3_left_encoder", Float32, queue_size=10)
    

    GPIO.output(DIR_PIN, GPIO.HIGH)  # Set initial motor direction (towards hardware)
    sensor_flag = 0

    left = GPIO.input(SENSOR_5) # left is 0 right is 525
    while not left:
        step()
